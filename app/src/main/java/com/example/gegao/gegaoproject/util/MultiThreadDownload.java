...
//==========================文件未下载完毕==========================
        String downloadResumedFolderName = MD5Utils.getMD5String(UrlStr);
        downloadResumedFolder = new File(FileConstants.WORK_FOLDER_SONGS_RESUME, downloadResumedFolderName);
        if (!downloadResumedFolder.exists()) {
            downloadResumedFolder.mkdirs();
        }
        ...
        try {
            ...
            for (int i = 0; i < threadNum; i++) {
                ...
                int currentThreadDownloadstartPosition = FileUtil.readThreadDownLoadInfo(downloadResumedFolder.getAbsolutePath(), "thread" + i + ".txt");
                int curThreadEndPosition = (i + 1) != threadNum ? ((i + 1) * blockSize - 1) : fileSize - 1;

//                FileDownloadThread fdt = new FileDownloadThread(url, saveFile,
//                        i * blockSize ,
//                        curThreadEndPosition,
//                        0);
                FileDownloadThread fdt = new FileDownloadThread(url, saveFile,
                        currentThreadDownloadstartPosition == 0 ? i * blockSize : currentThreadDownloadstartPosition,
                        curThreadEndPosition,
                        currentThreadDownloadstartPosition == 0 ? i * blockSize : currentThreadDownloadstartPosition - i * blockSize,
                        downloadResumedFolder.getAbsolutePath());
                fdt.setName("thread" + i + ".txt");
                fdt.start();
                fds[i] = fdt;
            }
            /**
            ...
            while (!finished) {
                ...
                sleep(100);/* 0.1秒钟刷新一次界面 */
                sendMsg(FileUtil.updateDownloadMeg, downloadPercent);
            }

            completed = true;
            FileUtil.deleteFilesByDirectory(downloadResumedFolder);
